...
@section('content')
    ...
    <div class="topbar">
        ...
        <div class="badge">{{ $guru->jadwal->count() }} Jadwal</div>
    </div>

    @if($guru && $guru->jadwal->count())
        @foreach($guru->jadwal as $j)
            <div class="card jadwal-card">
                <div class="jadwal-header">
                    <div class="jadwal-info">
                        <h3 class="jadwal-title">{{ $j->mapel->nama_mapel ?? '-' }}</h3>
                        <p class="jadwal-subtitle">Kelas {{ $j->kelas->nama_kelas ?? '-' }}</p>
                    </div>

                    <div class="badge">{{ $j->siswa->count() }} Siswa</div>
                </div>

                <div class="jadwal-stats">
                    <div class="stat-item">
                        <div class="stat-label">Tanggal</div>
                        <div class="stat-value">{{ \Carbon\Carbon::parse($j->tanggal)->format('d/m/Y') }}</div>
                    </div>

                    <div class="stat-item">
                        <div class="stat-label">Waktu</div>
                        <div class="stat-value">{{ $j->jam_mulai }} - {{ $j->jam_selesai }}</div>
                    </div>

                    <div class="stat-item">
                        <div class="stat-label">Durasi</div>
                        <div class="stat-value">{{ \Carbon\Carbon::parse($j->jam_mulai)->diffInHours(\Carbon\Carbon::parse($j->jam_selesai)) }} jam</div>
                    </div>

                    <div class="stat-item">
                        <div class="stat-label">Hari</div>
                        <div class="stat-value">{{ \Carbon\Carbon::parse($j->tanggal)->format('l') }}</div>
                    </div>
                </div>
                ...
                @if($j->siswa->count())
                    <div class="table-responsive">
                        <table class="siswa-table">
                            ...
                            <tbody>
                                @foreach($j->siswa as $index => $siswa)
                                    <tr>
                                        <td class="no-column">{{ $index + 1 }}</td>
                                        <td class="nama-column">{{ $siswa->nama_siswa }}</td>
                                        <td class="wa-column">{{ $siswa->no_whatsapp }}</td>
