...
<div class="topbar">
    <div>
        ...
        <p>Pantau perkembangan belajar dan administrasi <strong>{{ $siswa->nama_siswa }}</strong> secara real-time.</p>
    </div>
    <div class="badge">
...
<div class="grid">
    <div class="card stat" style="border-left: 5px solid #ffc107;">
        <div>
            ...
            <strong>{{ number_format($rataNilai, 1) }}</strong>
        </div>
    ...
    <div class="card stat" style="border-left: 5px solid #673ab7;">
        <div>
            <h3>Mapel Terbaik</h3>
            <strong style="font-size: 16px;">{{ $mapelTerbaik }}</strong>
        </div>
    ...
    <div class="card stat" style="border-left: 5px solid #197f48;">
        <div>
            <h3>Total Lunas</h3>
            <strong>Rp {{ number_format($lunas, 0, ',', '.') }}</strong>
        </div>
...
<div class="main-grid">
                ...
                <table style="width: 100%; border-collapse: collapse; margin-top: 10px;">
                    ...
                    <tbody>
                        ...
                        <tr style="border-bottom: 1px solid #eef3f8;">
                            ...
                            <td style="padding: 15px 10px;">
                                <span class="badge" style="background: {{ $n->nilai >= 75 ? '#e9f7ef' : '#fdecea' }}; color: {{ $n->nilai >= 75 ? '#197f48' : '#d32f2f' }}; padding: 5px 12px;">
                                    {{ $n->nilai }}
                                </span>
                            </td>
    ...
    <div class="card">
        ...
        @if($pembayaranTerakhir->count() > 0)
            @foreach($pembayaranTerakhir as $p)
            <div style="display: flex; justify-content: space-between; align-items: center; padding: 15px 0; border-bottom: 1px solid #eef3f8;">
                <div>
                    <div style="font-weight: 700; color: #072d54;">{{ $p->bulan }}</div>
                    <div style="font-size: 12px; color: #7b8490;">Jatuh Tempo: {{ date('d M Y', strtotime($p->tanggal_jatuh_tempo)) }}</div>
                </div>
                <div style="text-align: right;">
                    <div style="font-weight: 800; color: #072d54; margin-bottom: 5px;">Rp {{ number_format($p->jumlah, 0, ',', '.') }}</div>
                    <span style="font-size: 11px; padding: 4px 10px; border-radius: 99px; font-weight: 800; 
                        @if($p->status == 'Lunas') background: #e9f7ef; color: #197f48; 
                        @elseif($p->status == 'Menunggu') background: #fff8e1; color: #ffc107;
                        @else background: #fdecea; color: #d32f2f; @endif">
                        {{ strtoupper($p->status) }}
                    </span>
                </div>
            </div>
