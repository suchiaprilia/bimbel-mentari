        ...
        <div style="display: flex; justify-content: space-between; align-items: flex-start; flex-wrap: wrap; gap: 20px;">
            <div>
                ...
                <div class="stat-pill">
                    ...
                    <span>Total Tagihan: <strong>Rp {{ number_format($totalTagihan, 0, ',', '.') }}</strong></span>
                </div>
            </div>
                    ...
                    <table class="table-premium">
                        ...
                        <tbody>
                            @forelse($pembayaran as $item)
                                <tr>
                                    <td>
                                        <div style="font-weight: 800; color: #072d54;">{{ \Carbon\Carbon::parse($item->tanggal_jatuh_tempo)->translatedFormat('F Y') }}</div>
                                        <small style="color: #6f7b8a; font-weight: 600;">JT: {{ \Carbon\Carbon::parse($item->tanggal_jatuh_tempo)->format('d/m/Y') }}</small>
                                    </td>
                                    <td style="font-weight: 800; color: #003b70;">Rp {{ number_format($item->jumlah, 0, ',', '.') }}</td>
                                    <td>
                                    ...
                                    <td>
                                        <small style="color: #6f7b8a; font-weight: 600;">{{ ucfirst($item->metode_pembayaran ?? 'Cash') }}</small>
                                    </td>
                                </tr>
                            @empty
                                <tr>
                            ...
                            <select name="id_pembayaran" class="form-control" style="border-radius: 14px; height: auto; padding: 12px; border: 1px solid #d8e0ea; background: #f8fafc;" required>
                                @foreach($unpaid as $item)
                                    <option value="{{ $item->id }}">
                                        {{ \Carbon\Carbon::parse($item->tanggal_jatuh_tempo)->translatedFormat('F Y') }} - Rp{{ number_format($item->jumlah, 0, ',', '.') }}
                                    </option>
                                @endforeach
                            </select>
                        </div>
